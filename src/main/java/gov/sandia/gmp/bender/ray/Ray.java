...
//	}
	...
	/**
	 * Retrieve the azimuth observed at receiver looking along the ray toward the
	 * source.
	 * 
	 * @return double
	 */
...
//			  segment.getRayParameterInfo(rayParameters, origin, property);
	 ...
	 *          on the ray
	 * @param onCenters
	 *          boolean if onCenters is true, then new points are positioned at
	 *          the center of each interval. If false, then first point is a clone
	 *          of the source, last point is clone of receiver and other points
	 *          are evenly spaced in between.
	 * @param samples
	 *          list of InterpolatedNodeLayered objects that are equally spaced
	 *          along the ray.
	 * @param dkm
	 *          list containing the length of each ray segment surrounding the
	 *          samples. Ostensibly, these will will be equal to dkmMax (or
	 *          slightly less). They will be same in each RaySegment, but may be
	 *          slightly different in different RaySegments.
	 * @throws GeoModelException
	 * @throws GeoTessException
	 */
	public void resample(double dkmMax, boolean onCenters,
			ArrayList<GeoTessPosition> samples, ArrayListDouble dkm)
			throws GeoTessException
	{
		samples.clear();
		dkm.clear();

		// generous estimate of the required capacity for samples and dkm
		int n = 2 * getNSegments();
		for (RayBranch branch : rayBranches)
		{
			RaySegment segment = branch.firstActiveSegment;
			while (segment != branch.lastActiveSegment.nextActiveSegment)
			{
			  n += (int) Math.ceil(segment.getPathLength() / dkmMax);
				segment = segment.nextActiveSegment;
			}
		}
//		  for (RaySegment segment : branch.getBranchSegments())
//			  n += (int) Math.ceil(segment.getPathLength() / dkmMax);
		...
		samples.ensureCapacity(n);
		dkm.ensureCapacity(n);
		...
		for (RayBranch branch : rayBranches)
		{
			...
			while (segment != branch.lastActiveSegment.nextActiveSegment)
			{
				segment.resample(dkmMax, onCenters, samples, dkm);
				segment = segment.nextActiveSegment;
			}
		}
//		for (int i = 0; i < branch.getBranchSegments().size(); ++i)
//			branch.getBranchSegments().get(i).resample(dkmMax, onCenters, samples, dkm);
	}
