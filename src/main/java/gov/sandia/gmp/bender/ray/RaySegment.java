...
//	protected int branchSegmentIndex = -1;
	...
	/**
	 * The waveType of this RaySegment: either PSLOWNESS or SSLOWNESS.
	 */
	protected GeoAttributes waveType;

	/**
	 * The attributeIndex of waveType
	 */
	protected int waveTypeIndex;
...
//	protected Statistic nodeMovementStats = null;
   ...
   */
	...
	public RaySegment(Ray ray, RayBranch branch, LinkedList<GeoTessPosition> nodes,
			               int waveIndex) throws GeoTessException
	{
		initializeSegment(ray, branch, nodes, waveIndex, null);
	}

	public RaySegment(Ray ray, RayBranch branch, LinkedList<GeoTessPosition> nodes,
										int waveIndex, RaySegment prevSegment) throws GeoTessException
	{
		initializeSegment(ray, branch, nodes, waveIndex, prevSegment);
  }
	...
	private void initializeSegment(Ray ray, RayBranch branch,
																 ...
																 int waveIndex, RaySegment prevSegment)
	{
		...
		waveTypeIndex= waveIndex;
		waveType = GeoAttributes.valueOf(ray.getGeoTessModel().getMetaData().getAttributeName(waveIndex));
...
//	}
	...
	/**
	 * Retrieve the azimuth between last and next-to-last point on the ray
	 * in this segment.
	 * 
		...
		try
		{
			...
			while (iterator.hasNext())
			{
				...
				buffer.append(String.format("%10.4f %10.4f %10.4f %10.4f  %10.4f   %4d %12s%n",
						...
						node.getDepth(),
						node.getValue(waveTypeIndex, majorLayerIndex),
						node.getIndex(),
	...
	protected double getSegmentTravelTime() throws GeoTessException
	{
		if (travelTime < 0.0)
		{
			...
			double dkm, slow2, slow1 = previous.getValue(waveTypeIndex, majorLayerIndex);
	      ...
	      pathLength += dkm;
				...
				slow2 = node.getValue(waveTypeIndex, majorLayerIndex);
		...
		while (iterator.hasNext())
		{
			...
			if (ray.bender.getGradientCalculatorMode() == GradientCalculationMode.ON_THE_FLY)
				ray.bender.getGradientCalculator().getGradient(node.getVector(),
						       node.getRadius(),  waveTypeIndex,  majorLayerIndex,
						       true,  gradient);
			else
			  node.getGradient(waveTypeIndex, majorLayerIndex, true, gradient);
	 ...
	 * interval is no larger than dkmMax.  Set dkmMax = this new interval length.
	 * If the path length in this segment is very small simply return.
	 * Find a point dkmMax/2 distance from the start of the ray segment
	 * and create a new interpolated node there.  Add the node to the 
	 * array of samples.  Then move along the ray segment distance dkmMax
	 * and add another node to samples.  Continue this process until
	 * the end of the ray segment is reached.  Note that samples are 
	 * created at the centers of the sub intervals.  No samples are
	 * created at the endpoints of the ray segment.
	 * @param dkmMax
	 * @param samples
	 * @param dkm
	 * @throws GeoTessException 
	 */
	public void resample(double dkmMax, boolean onCenters,
					ArrayList<GeoTessPosition> samples, 
					ArrayListDouble dkm) throws GeoTessException
	{
		...
		// set nSamples = number of intervals
		int nSamples = (int)Math.ceil(getPathLength()/dkmMax);

		// update dkmMax to actual interval size in km.
		dkmMax = getPathLength() / nSamples;

		// if onCenters, then first new node will be dkmMax/2 away from 
		// first node of this segment, otherwise start will coincide with
		// first node of this segment.
		double start = onCenters ? dkmMax/2 : 0;
		double increment;

		GeoTessPosition sample = null;
		...
		while (iterator.hasNext())
		{
			...
			while (start < increment)
			{
				...
				sample = GeoTessPosition.getGeoTessPosition(previous,  next, start / increment);

				if (onCenters || sample.getDistance3D(getLast()) > dkmMax * 0.1)
					// add the new node to list of samples
					samples.add(sample);
		...
		if (!onCenters && segmentIndex == branch.getNSegments() - 1)
		{
			sample = getLast().deepClone();
			samples.add(sample);
		}
	}
