...
//	}
	...
	protected void bendNode() throws GeoTessException
	{
		...
		if (Vector3D.lengthSquared(gradient) > 1e-16)
		{
			...
			vmid = 1. / node.getValue(waveTypeIndex, majorLayerIndex);
			...
			c = 0.5 * previous.getValue(waveTypeIndex, majorLayerIndex) +
			    0.5 * next.getValue(waveTypeIndex, majorLayerIndex);
