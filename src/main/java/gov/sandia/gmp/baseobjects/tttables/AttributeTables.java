...
//		}
		...
		for (SeismicPhase phase : EnumSet.allOf(SeismicPhase.class))
			if (new SeismicBaseData(new File(directory, phase.getFileName())).exists())
	 ...
	 */
	public double getValue(GeoAttributes attribute, PredictionInterface prediction)
			throws OutOfRangeException, IOException, GMPException {
		return getValue(attribute, prediction.getPhase(), prediction.getReceiver().getPosition(),
				prediction.getSource().getPosition());
	}
