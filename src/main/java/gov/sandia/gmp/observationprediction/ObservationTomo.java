...
//import gov.sandia.gmp.bender.RayInfo;
...
public class ObservationTomo
       implements Serializable, Cloneable, PredictionRequestInterface
{
  /**
...
//
//    /**
//     * Returns true if this observation is to be completely removed from all
//     * consideration.
//     * 
//     * @return True if this observation is to be completely removed from all
//     * consideration.
//     */
//    public boolean ignorePrediction()
//    {
//      if (ignoreTomography() && !ignoreTomographyCurrentIteration())
//        return true;
//      else
//        return false;
//    }
//
//    /**
//     * Returns true if this observation is not to be used in tomography for the
//     * current iteration.
//     * 
//     * @return True if this observation is not to be used in tomography for the
//     *         current iteration.
//     */
//    public boolean ignoreTomographyCurrentIteration()
//    {
//      if (this.name().startsWith("REMOVE_ITERATION"))
//        return true;
//      else
//        return false;
//    }
//
//    /**
//     * Returns true if this observation status is not to be used in tomography.
//     * 
//     * @return True if this observation status is not to be used in tomography.
//     */
//    public boolean ignoreTomography()
//    {
//      if (this != USE_TOMOGRAPHY)
//        return true;
//      else
//        return false;
//    }
  ...
  /**
   * The location of the seismic source.  Normally based on orign.lat, origin.lon
   * and origin.depth.
   */
  private Source source;

  /**
   * The location of the seismic receiver.  If station information comes from
   * a ttmodel database table, this is based on ttmodel.stalat, ttmodel.stalon and
   * ttmodel.staelev.  If information comes from a site table, then it is based
   * on ste.lat, site.lon and site.elev.
   */
  private Receiver receiver;

  /**
   * The prediction associated with this observation.
   */
  private PredictionInterface prediction;

  /**
   * All the database ID numbers associated with this observation.
   */
  private long observationId, predictionId;

  /**
   * uniqueSta is a string that uniquely identifies a seismic station.  If
   * station information is obtained from a ttmodel database table, then
   * ttmodel.sta can be used for the this purpose.  If station information
   * comes from a site table, then site.sta_site.ondate would be unique.
   */
  //private String uniqueStationName;

  /**
   * The seismic phase of this observation
   */
...
//
//  /**
//   * The first iteration of the current adaption level residual travel time.
//   * This value is reset 
//   */
//  private double ttFirstAdaptResidual = -1.0;
  ...
  private EnumSet<GeoAttributes> requestedAttributes;
  ...
  private int[]    aTomoWeightIndexes = null;
  private double[] aTomoWeights     = null;
  ...
  {
  	prediction 				 = pr.getPrediction();
    aTomoWeightIndexes = pr.getWeightIndexArray();
    aTomoWeights			 = pr.getWeightArray();
  }

  public int[] getTomoWeightIndexes()
  {
  	return aTomoWeightIndexes;
  }

  public double[] getTomoWeights()
  {
  	return aTomoWeights;
  }
      ...
      int index)
  {
    ...
    this.requestedAttributes = requesteAttributes;
...
//  /**
//   * Retrieve the name of the SeismicPhase
//   * @return String
//   */
//  public String getPhaseName()
//  {
//    return phase.getPhaseName().toString();
//  }
  ...
  /**
   *
   * @return Prediction associated with this observation.
   */
  ...
  /**
   * Returns the distance (in degrees) between the source and the receiver.
   * 
   * @return The distance (in degrees) between the source and the receiver.
   */
  public double getDistance()
  {
    return receiver.distance(source);
  }
  ...
  /**
   * Set the prediction result for this observation.
   * 
   * @param rayInfo RayInfo
   */
  public void setPrediction(PredictionInterface p)
  {
    prediction = p;
  }
...
//  }
  ...
  public Object[] getSourceForDb()
  {
	...
	values[2] = source.getPosition().getLatDegrees();   // lat
	values[3] = source.getPosition().getLonDegrees();   // lon
	values[4] = source.getPosition().getDepth();   // depth
  ...
  {
    Object values[]  = new Object[11];
    GeoVector srcgv  = source.getPosition();
    GeoVector rcvrgv = receiver.getPosition();
    values[0] = source.getSourceId();   // sourceid
  ...
  public Object[] getReceiverForDb()
  {
    ...
    values[0] = receiver.getReceiverId(); // receiverid  // TODO use timestamp instead
    values[1] = receiver.getSta();   // sta
    values[2] = receiver.getPosition().getLatDegrees();   // lat
    values[3] = receiver.getPosition().getLonDegrees();   // lon
    values[4] = -receiver.getPosition().getDepth();   // elevation
 ...
 * @param  
		  ...
		  ScreenWriterOutput scrnWrtr)
  {
    ...
    if(polygon != null) {
    	if(polygon.contains(source.getPosition().getUnitVector()))
    		s.append("s");
    	if(polygon.contains(receiver.getPosition().getUnitVector()))
    		s.append("r");
...
//  }
  ...
  {
    return receiver.distanceDegrees(source);
  }
	 ...
	 * boolean flag is returned which indicates the existance or absence of an
    ...
    if (n == 0)
    {
    	aTomoWeightIndexes = null;
    	aTomoWeights       = null;
    }
    else
    {
      aTomoWeightIndexes = fib.readInts();
      aTomoWeights       = fib.readDoubles();
    }
    ...
    if (aTomoWeights == null)
    	fob.writeInt(0);
    else
    {
    	fob.writeInt(aTomoWeights.length);
      fob.writeInts(aTomoWeightIndexes);
      fob.writeDoubles(aTomoWeights);
    }
